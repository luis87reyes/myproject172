<?php 

require_once('../../../private/initialize.php'); 

// Check if request contains an ID, if not then redirect
if (!isset($_GET['id'])) {
  redirect_to(url_for('/Staff/Salaries/index.php'));
}
$id = $_GET['id'];

if (is_post_request()) {

    // Handle form values sent by new.php
    $salary = [];
    $salary['emp_no'] = $_POST['emp_no'] ?? '';
    $salary['salary'] = $_POST['salary'] ?? '';
    $salary['from_date'] = $_POST['from_date'] ?? '';
    $salary['to_date'] = $_POST['to_date'] ?? '';

    $result = update_salary($salary);
    $_SESSION['message'] = 'The Salary was updated successfully.';
    redirect_to(url_for('/Staff/Salaries/show.php?id=' . $id));

} else {

  $salary = find_salary_by_emp_no($id);

}
?>

<?php $page_title = 'Edit Salary'; ?>
<?php include(SHARED_PATH . '/staff_header.php'); ?>

<div id="content">

  <a class="back-link" href="<?php echo url_for('/Staff/Salaries/index.php'); ?>">&laquo; Back to List</a>

  <div class="salary edit">
    <h1>Edit Salary</h1>

    <form action="<?php echo url_for('/Staff/Salaries/edit.php?id=' . h(u($id))); ?>" method="post">
      <dl>
        <dt>Employee Number</dt>
        <dd><?php echo h($salary['emp_no']); ?></dd>
      </dl>
      <dl>
        <dt>Salary</dt>
        <dd><input type="text" name="salary" value="<?php echo h($salary['salary']); ?>" /></dd>
      </dl>
      <dl>
        <dt>Beginning Date</dt>
        <dd><input type="text" name="from_date" value="<?php echo h($salary['from_date']); ?>" /></dd>
      </dl>
      <dl>
        <dt>End Date</dt>
        <dd><input type="text" name="to_date" value="<?php echo h($salary['to_date']); ?>" /></dd>
      </dl>
      <div id="operations">
        <input type="submit" value="Edit Salary" />
      </div>
    </form>

  </div>

</div>

<?php include(SHARED_PATH . '/staff_footer.php'); ?>
